...
import com.tasksystemnidharshana.taskmanager.payload.ErrorDetail;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    //invalid status or priority error
    @ExceptionHandler(TaskApiException.class)
    public ResponseEntity<ErrorDetail> handleTaskApiException(
            TaskApiException ex, WebRequest request) {
        ErrorDetail errorDetail = new ErrorDetail(
                new Date(), ex.getMessage(), request.getDescription(false));
        return ResponseEntity.status(ex.getStatus()).body(errorDetail);
    }
    // when compulsory fields are not filled
    @Override
